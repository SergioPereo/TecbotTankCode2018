 ...
 *
 */
public class ArmThirdPosition extends Command {
	boolean isInTheRightPosition, isTheCommandFinished = false;

    public ArmThirdPosition() {
    ...
    protected void execute() {
    	if (!isInTheRightPosition) {
			if (Robot.arm.getUpEncoder().get() > Robot.arm.getHighPosition()
					&& Robot.arm.getDownEncoder().get() > Robot.arm.getDefaultPosition()) {
				Robot.arm.armDOWN();
				isTheCommandFinished = false;
			} else if (Robot.arm.getUpEncoder().get() < Robot.arm.getHighPosition()
					&& Robot.arm.getDownEncoder().get() < Robot.arm.getHighPosition()) {
				Robot.arm.armDOWN();
				isTheCommandFinished = false;
			} else if (Robot.arm.getUpEncoder().get() < Robot.arm.getHighPosition()
					&& Robot.arm.getDownEncoder().get() > Robot.arm.getHighPosition()) {
